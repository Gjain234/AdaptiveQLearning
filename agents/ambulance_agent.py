...
from trees import ambulance_tree
import numpy as np

class AmbulanceAgent(adaptive_Agent.AdaptiveDiscretization):
    ...
    def __init__(self, epLen, numIters, scaling):
        ...
        for h in range(epLen):
            tree = ambulance_tree.AmbulanceTree(epLen)
    ...
    def reset(self):
        ...
        for h in range(self.epLen):
            tree = ambulance_tree.AmbulanceTree(self.epLen)
            self.tree_list.append(tree)
