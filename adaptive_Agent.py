...
from tree import Tree, Node, Tree_Multiple_Ambulance, Node_Multiple_Ambulance
...
class AdaptiveDiscretizationMultiple(agent.FiniteHorizonAgent):
    ...
    def __init__(self, epLen, numIters, scaling):
        ...
        # Makes a new partition for each step and adds it to the list of trees
        for h in range(epLen):
            tree = Tree_Multiple_Ambulance(epLen)
            self.tree_list.append(tree)
    ...
    def reset(self):
        ...
        self.tree_list = []
        for h in range(self.epLen):
            tree = Tree_Multiple_Ambulance(self.epLen)
            self.tree_list.append(tree)
